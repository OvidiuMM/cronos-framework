...
**Provenance:** Reconstructed from the toolkit conventions proven across production cycles 0001–0009; structure generalized for public use.
...
├── skills/      # Mechanized automations (scripts, agent skills, CI checks).
│                # Promoted when a lesson must be ENFORCED, not remembered —
│                # this is where Mandate 6 (Recurrence Rule) conversions land.
└── knowledge/   # Reference documents: patterns, platform gotchas, standards.
                 # Gated by the 3-cycle rule (see §4).
```
...
- **Working entries:** the prompt, what it achieved, one line of context.
...
## 5. Consumption: how assets flow back into cycles
